...
static void emitReturn() {
  if (current->type == TYPE_INITIALIZER) {
    // For method, slot 0 contains "this"/instance.
    // Hence, when we return from init method we always return instance
...
static void initCompiler(Compiler *compiler, FunctionType type) {
  ...
  // While lexical scoping, we keep first slot reserved for function
  // name.
...
static void declareVariable() {
  ...
  for (int i = current->localCount - 1; i >= 0; i--) {
    ...
    if (identifiersEqual(name, &local->name)) {
      error("Already a variable with this name exists in this scope");
    }
  }
  addLocal(*name);
}
...
static void super_(bool canAssign) {
  ...
  consume(TOKEN_IDENTIFIER, "Expect superclass method name after 'super.'.");
  ...
  if (!check(TOKEN_RIGHT_PAREN)) {
    do {
      ...
      if (current->function->arity > 255) {
        errorAtCurrent("Can't have more the 255 function parameter");
      }

      uint8_t constant = parseVariable("Expect function parameters name");
...
static void classDeclaration() {
  ...
  // added in global table
  declareVariable();

  /*In global table we store class_name = classObj
   * In global case: creates the object with name, and push Obj to stack,
   * and define the global value with the same name.
   * OP_CLASS
   * constant_table INDEX
   * OP_DEFINE_GLOBAL
   * constant_table INDEX
   *
   * In Local Case, we don't have the OP_DEFINE_GLOBAL, but value is on top
   * of the stack of where its expected to be.
   */
  ...
  if (match(TOKEN_LESS)) {
    ...
    // Read previous parsed token, and adds it onto stack
    ...
    // Adds the passed token onto stack
...
static void funDeclaration() {
  ...
  // NOTE: If function is a global, we store it in globals tables, hence it is
  // resolved at runtime, we store the function name identifier in constant
  // table, and get the index while storing it in table. Its same as how we
  // declare the global variable. Same with functions inside a function, which
  // are locally defined. Here, unlike local where we don't support access the
  // variable being declared like var a = a; For functions, we can allow this,
  // this is how we can support "recurision". Hence unlike local variable, where
  // declaration and definition is done in 2 steps, where we mark it as
  // initialized after defining, here we mark intialized after we parse the name
  markInitialized();
...
static void forStatement() {
  ...
  if (!match(TOKEN_RIGHT_PAREN)) {
    ...
    expression();
    emitByte(OP_POP);
    consume(TOKEN_RIGHT_PAREN, "Expect ')' after the for clauses.");

    // After increment jump to starting of the condition
    emitLoop(loopStart);
  ...
  statement();
  // Jump Loop to the increment after the body if there is a increment/decrement
  // condition
  emitLoop(loopStart);
